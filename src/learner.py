...
class Learner(GetAttr):
    ...
    def __init__(self, dls, model,
        ...
        # self.scaler = self.dls.scaler
        # self.mean = torch.from_numpy(self.dls.scaler.mean_)
        # self.std = torch.from_numpy(self.dls.scaler.scale_)
    ...
    def train_step(self, batch):
        ...
        # self.yb = self.yb * self.std + self.mean
        # compute loss
    ...
    def test(self, dl, weight_path=None, scores=None):
        ...
        # print('preds', cb.preds.shape)
        # print('targets', cb.targets.shape)
        # mean = self.scaler.mean_
        # std = self.scaler.scale_
        
        
        self.preds, self.targets = to_numpy([cb.preds, cb.targets])
        # cb.preds = cb.preds.to('cpu') * std + mean
        # cb.targets = cb.targets.to('cpu') * std + mean
